...
//		$inputObj->SetMch_id(WxPayConfig::$MCHID);//商户号
    ...
    public static function mchPay($inputObj)
    {
        ...
        if (!$inputObj->IsAmountSet()) {
            ...
            throw new WxPayException("企业付款接口中， 缺少必填参数check_name_set");
        ...
        } else if (!$inputObj->IsSetPartnerTradeNo()) {
            throw new WxPayException("企业付款接口中， 缺少必填参数partner_trade_no");
        }
        $inputObj->SetMchAppId(config("weixin.app_id"));
        $inputObj->SetMchId(config("weixin.mch_id"));
        $inputObj->SetSpillCreateIp($_SERVER['REMOTE_ADDR']);//终端ip
    ...
    /**
     * 企业付款
     * @param WxMchPayBank $inputObj
     * @return mixed
     * @throws WxPayException
     */
    public static function mchPayBank($inputObj)
    {
        $url = "https://api.mch.weixin.qq.com/mmpaysptrans/pay_bank";
        if (!$inputObj->IsAmountSet()) {
            throw new WxPayException("企业付款接口中， 缺少必填参数amount");
        } else if (!$inputObj->IsBankNo()) {
            throw new WxPayException("企业付款接口中， 缺少必填参数enc_bank_no");
        } else if (!$inputObj->IsDescSet()) {
            throw new WxPayException("企业付款接口中， 缺少必填参数desc");
        } else if (!$inputObj->IsTrueName()) {
            throw new WxPayException("企业付款接口中， 缺少必填参数enc_true_name");
        } else if (!$inputObj->IsSetPartnerTradeNo()) {
            throw new WxPayException("企业付款接口中， 缺少必填参数partner_trade_no");
        } else if(!$inputObj->IsBankCode()){
            throw new WxPayException("企业付款接口中， 缺少必填参数bank_code");
        }
        $inputObj->SetMchId(config("weixin.mch_id"));
        $inputObj->SetNonceStr(self::getNonceStr());
        $inputObj->SetSign();
